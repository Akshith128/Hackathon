...
export default function AdminDashboard() {
  ...
  const handlePromotionEval = async () => {
    try {
      const res = await fetch('/api/admin/evaluate-promotion', { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
        toast({
          title: "Promotion Evaluation Complete",
          description: `Promoted: ${data.promoted}, Detained: ${data.detained}`,
        });
      }
    } catch (e) {
      toast({ title: "Error", description: "Failed to evaluate promotion", variant: "destructive" });
    }
  };
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <Button onClick={handlePromotionEval} variant="outline" className="gap-2">
            <ShieldCheck className="w-4 h-4" /> Run Promotion Check
          </Button>
